//  SessionsView.swift
//  Your cascade sessions, enumerated by the daemon (GET /sessions). Tap to
//  attach; long-press for color tags or removal; + spawns a new one.
...
struct SessionsView: View {
    ...
    var body: some View {
        Text("SESSIONS DEBUG \(app.sessions.count)")
            .font(.largeTitle)
            .foregroundStyle(.red)
...
struct JoinedCard: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            // 46pt Liquid Glass icon tile with etched Cascade logo + mode badge
