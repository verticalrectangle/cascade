...
//  live CascadeClient. Local updates only — no push token is requested here, so it
...
final class LiveActivityController {
    private var activity: Activity<CascadeActivityAttributes>?

    static func state(from c: CascadeClient) -> CascadeActivityAttributes.ContentState {
        let waiting = c.turns.contains { $0.type == .ask }
        let status = CascadeStatus.from(phase: c.phase, working: c.working, waiting: waiting)
    ...
    func sync(sessionId: String, state: CascadeActivityAttributes.ContentState) {
        guard ActivityAuthorizationInfo().areActivitiesEnabled else { return }
        if let activity {
            Task { await activity.update(ActivityContent(state: state, staleDate: nil)) }
            return
        }
        let attributes = CascadeActivityAttributes(sessionId: sessionId)
