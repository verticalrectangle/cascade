...
struct TurnRow: View {
    ...
    /// cascade asks make them available (callbacks non-nil).
...
/// Plan approval / refinement card for plan mode. The full plan markdown is
...
/// The thinking state: just the cascade eye, its pupil dilating and closing on a
/// slow breath — no text. Clean, single-glyph, unmistakably Cascade.
...
struct LogoMark: View {
    ...
    var body: some View {
        ZStack {
            ...
            CascadeSlit(open: open).stroke(color, style: StrokeStyle(lineWidth: size * 0.06, lineCap: .round, lineJoin: .round))
        }
